...
def private_runtime_lock_dir(*, base: Path | None = None) -> Path:
    ...
    Prefers ``$XDG_RUNTIME_DIR/kdive/locks``; otherwise falls back to
    ``<base>/kdive-<uid>/locks``. Both branches enforce the same symlink,
    ...
    if runtime_dir:
        base_dir = Path(runtime_dir) / "kdive"
    else:
        root = base if base is not None else Path(tempfile.gettempdir())
        base_dir = root / f"kdive-{os.getuid()}"
...
def private_runtime_registry_dir(*, base: Path | None = None) -> Path:
    ...
    Sibling of the lock dir: ``$XDG_RUNTIME_DIR/kdive/registry`` (or the
    ``<base>/kdive-<uid>/registry`` fallback), with the same symlink/owner/0700
