...
- **Modify** `src/linux_debug_mcp/server.py` — `DEBUG_METHOD_OPERATIONS` gains `load_module_symbols`; `_read_module_sections` (SSH-sysfs) + `_default_module_ko_finder` + `debug_load_module_symbols_handler` + tool wrapper; `is_lossy_out_of_band` + wire warning into `debug_start_session_handler`; break-entry router consulting `TransportSession.break_plan`; per-op `transport_stall` teardown; `transport_quality_warning` redaction.
...
- [ ] **Step 1 — failing tests** (handler-level, inject a `gdb_mi_engine` whose op raises `GdbMiError(transport_stall)` and a fake transaction/admission/registry):
  - `test_transport_stall_reaps_resumes_and_tears_down`: assert the response is `INFRASTRUCTURE_FAILURE` with `details["code"]=="transport_stall"`, `suggested_next_actions` contains `debug.start_session`, `debug.kdb`, `debug.introspect.run`; assert the attachment was reaped, `force_resume` called, the durable record written EXECUTING, and the guard released (via fakes recording calls).
...
- [ ] **Step 2 — run red.**
- [ ] **Step 3 — implement.** Inside the inner `try` (where the raw-fault branch already reaps), catch `GdbMiError` whose `details.get("code") == "transport_stall"` and run the same teardown sequence `_run_mi_attach_probe`'s failure path uses (`server.py:4111-4130`): `force_resume(reaped)` → `_resume_debug_transport(...)` (best-effort) → `_teardown_debug_transport(...)`, then re-raise/return the structured `INFRASTRUCTURE_FAILURE`. Plumb `transaction`/`session_guard` into `_debug_operation_response` and the per-op handlers if not already present (mirror `debug.start_session`'s wiring). All teardown happens before the `with store.debug_lock` block exits.
- [ ] **Step 4 — run green** (and the full `tests/test_server_debug_*` to confirm no regression).
...
- [ ] **Step 1 — failing tests** (pure router + handler-level):
  ...
  - `test_router_inject_for_serial_method`: a `break_plan.method == AGENT_PROXY_BREAK` routes to the inject seam; assert the inject is requested with the admitted method.
  - `test_router_missing_proxy_handle_is_break_inject_unavailable`: a non-native plan whose transport has no resolvable proxy handle → `CONFIGURATION_ERROR` / `break_inject_unavailable`, never a silent no-op.
- [ ] **Step 2 — run red.**
- [ ] **Step 3 — implement.** A pure `_break_entry_method(transport_session) -> BreakMethod` (load `TransportSession` by `transport_session_id` from `session_registry`; default `GDBSTUB_NATIVE` when record/plan absent). In the interrupt path, branch: native → engine `interrupt()`; else → `transaction.inject_break_for_session(session_id, method)` then `wait_for_stop`. The actual non-native execution (resolving the live proxy handle) is exercised only by Task 8's gated test; here assert the routing + the `break_inject_unavailable` guard.
- [ ] **Step 4 — run green.**
