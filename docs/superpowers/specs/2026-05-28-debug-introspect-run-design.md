...
agent ──MCP──▶ debug.introspect.run handler ──▶ AdmissionService.admit_ssh_tier
              ...
              host parses stdout      exit code          │
                            │                            │
                            ▼                            ▼
              Redactor → stdout.json   admission.complete(handle)
                            │
                            ▼
...
ToolResponse.success({
  ...
  "artifacts": [ArtifactRef, ...],      # request.json, stdout.json, stderr.log,
                                        # wrapper.skeleton.py — NOT wrapper.py / stdout.raw /
                                        # stderr.raw, which live under <run>/sensitive/ (§6.1, §6.3)
...
| Wrapper exit 3 with `outcome.status="drgn_version_skew"` (target drgn missing `main_module().build_id` or shaping it wrong) | `INFRASTRUCTURE_FAILURE` | `drgn_version_skew` | Wrapper |
...
try:
    ...
    # R3-F1: seed the snapshot with both wrapper-private names so the diff
    # excludes them. Assignment-order means the LHS name isn't bound in
    # `globals()` at the moment the RHS evaluates, so a naive
    # `set(globals().keys())` would capture neither `_li_pre_helpers` nor
    # `_li_drgn_helper_names`. After the wildcard import below `globals()`
    # would then contain `_li_pre_helpers`, and the post-import diff would
    # include it as a "new" name — which is what the §9.2
    # `test_wrapper_helper_namespace_contains_expected_subset` assertion
    # forbids. Pre-seeding excludes both wrapper-private names from the
    # diff and keeps them out of the user namespace.
...
| 6 | absent / unparseable, OR valid minimal JSON with `outcome.status="wrapper_internal_error"` | R2-F2: either the inner-except recovery wrote a minimal JSON document (serialization-stage failure — see §4.2) or both writes failed (pipe-stage fall-through; see §4.2 R3-F2 note). The two sub-cases are distinguishable on disk via `stdout.json` / `details.outcome_status` for forensics but collapse to one agent-facing code (R4-F3). | `INFRASTRUCTURE_FAILURE` / `wrapper_crash` — raw stderr persisted; minimal JSON (if any) persisted via the redactor to `stdout.json` |
...
12. **Host-side post-processing.** Re-use the per-call `Redactor` (configured in step 1) on every variable-length string in the response and the on-disk `stdout.json` / `stderr.log` (§6.3). Build `ArtifactRef`s for `request.json`, `wrapper.skeleton.py`, `stdout.json`, and `stderr.log`; **`wrapper.py`, `stdout.raw`, and `stderr.raw` are each referenced by an `ArtifactRef` in the step record with `sensitive=True` (see §6.2) but are omitted from the response's `artifacts` list**.
13. **Manifest record.** Open `ArtifactStore._manifest_lock()` in exclusive mode. Re-read the manifest under the lock. R2-F5: the call-budget invariant is **not** re-checked here — step 4a is the only budget gate, and the soft-cap framing in §5.3 documents the consequences of that choice. The only post-lock check that may fail is `call_id` collision: if `step_results[f"introspect:{call_id}"]` already exists, `RunManifest.append_step_result` raises `ValueError("step name already recorded: …")`, which `ArtifactStore.record_step_result` re-raises as `ManifestStateError(..., INFRASTRUCTURE_FAILURE)` and the handler's outer `except Exception` rolls back admission and re-raises — a UUIDv4 collision is treated as an internal bug that should never happen, not as a contract failure with its own error code. Otherwise call `ArtifactStore.record_step_result(step_result, append=True)`. The append-mode helper is the existing `record_step_result` (today the wrapper around `RunManifest.with_step_result`) extended with a new `append: bool = False` kwarg that switches the underlying call to a pure `RunManifest.append_step_result(step_result)` — which mirrors `RunManifest.with_step_result` at `manifest.py:60` and returns a new `RunManifest`. `append=True` (a) sets `step_results[name] = step_result` and (b) atomically rewrites `manifest.json` via `tmp + rename`; the `steps` list is intentionally **untouched** (`manifest.py:79-83`) — that list is the fixed planned-step manifest (`create_run`, `build`, `boot`, `run_tests`, `collect_artifacts`, `debug`), and introspect calls grow `step_results` under `introspect:<call_id>` keys but stay out of `steps`. Iter-2 finding 1 follow-on: because dynamic step-result keys are NOT in `steps`, the bundler at `_bundle_for_manifest` must walk `manifest.step_results` directly to find them — see §6.1 / the artifacts.collect notes. The default `append=False` path remains for replace-on-`force_*` semantics on the named-singleton steps (`build`, `boot`, `run_tests`, `debug`). The same retry-with-backoff posture as `_record_terminal_build_result` applies if the lock is transiently unavailable.
...
- **`request.json`** is the redacted `DebugIntrospectRunRequest`. Iter-1 finding 5: the `script` field is replaced with `"sha256:<hex>"` (the digest of the decoded user-script bytes) *before* the redactor walks the dict — the plaintext script lives only under `<run>/sensitive/debug/introspect/<call_id>/wrapper.py`, and `request.json` carries the digest only.
- **`wrapper.py`** contains the user script **verbatim** (base64 is transport encoding, not redaction — `base64.b64decode` is a one-line reversal). It is written under `<run>/sensitive/debug/introspect/<call_id>/` with mode `0600` and is **not** surfaced in the response's `artifacts` list; the response references `wrapper.skeleton.py` instead, which carries the rendered wrapper with the script body replaced by `# <user script: sha256:...; full source under sensitive/debug/introspect/<call_id>/wrapper.py>`. Operators with filesystem access to the run dir can reconstruct the original script from `wrapper.py`; this is the same trust posture as `<run>/sensitive/` everywhere else in the project. Iter-1 finding 5 changed the agent-visible script handling: `request.json` carries a `sha256:` digest, not the script source, so the redactor no longer needs to scrub script-pasted secrets from that file — secrets inside `wrapper.py` itself are not scrubbed and the file's confidentiality depends on filesystem permissions (the `0700` parent dir + `0600` file mode contract from §6.1).
...
- `test_redactor_applied_to_emits_and_snippets` (FakeSshRunner returns JSON containing the configured secret; assert redaction in `stdout.json` on disk and in the response)
...
Hypothesis-driven JSON-shape assertion on the wrapper's output: regardless of script outcome, `stdout.json` parses, has a `call_id`, an `outcome`, and a `truncated` map.
