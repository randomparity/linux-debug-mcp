...
- Introduce a small injectable `ProcessIdentityProbe` Protocol: `identity(pid) -> ProcessIdentity | None` (`{pid, start_time, argv0}`, `None` if the pid is gone) and `is_alive(pid) -> bool`. The production default is a **single `/proc` implementation**, extracted from the existing `_controller_identity` logic so there is **one shared impl, not two diverging copies**. Unit tests inject a fake; the real-agent-proxy integration tests stay Linux/tool-gated.
- **Listener-identity verification** uses the **portable protocol-framing probe** (the RSP port answers RSP framing) as the **primary, load-bearing** signal; the pid-owns-the-listener check is a **Linux belt-and-suspenders** via the same injectable probe and degrades to skipped/fake elsewhere. The **reap-safety** property rests on the start-time fingerprint of the backend's *own spawned pid* (always known from `Popen`), **not** on a listener→pid mapping — so reaping stays safe even where pid-ownership cannot be observed.
