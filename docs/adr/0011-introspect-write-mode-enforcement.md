...
### 5. The wrapper write-guard is a factored, unit-testable snippet threaded by an `allow_write` flag

`render_wrapper(...)` gains `allow_write: bool = False`, substituting a new `${ALLOW_WRITE_GUARD}` block. When `allow_write=false` the block installs a guard that intercepts `prog.write`; when `true` it is a no-op (writes flow to drgn, which itself fails on today's read-only targets — that is drgn's call, not ours).

The guard is a small pure-Python snippet held as a **module-level string constant** in `local_drgn_introspect.py`, so it is `exec`-able against a fake program in a unit test without drgn installed. It blocks `prog.write` while forwarding every read of the drgn `Program` protocol (`__getattr__`, `__getitem__`, `__contains__`, `__call__`, `__iter__`, `__len__`). A `write_mode_disabled` outcome is emitted by catching the guard's sentinel exception around the user-script `exec`. AC#1 (read-only script identical false-vs-true) is the regression contract on the guard's transparency and is verified both by the snippet unit test and the env-gated live integration test.

**Known limitation (documented, not a regression):** drgn C-level constructors that type-check their `Program` argument (e.g. `drgn.Object(prog, …)`) reject a Python proxy. The guard therefore exposes the real `Program` for `prog`-as-argument call sites and wraps only the `prog.write` attribute path it can intercept; the residual bypass (calling the unwrapped program's `write` via the module/type) is acceptable because the *policy gate* — not the guard — is the boundary. This is stated in the spec's security section so it is never mistaken for a sandbox.

### 6. Audit trail: manifest detail + a dedicated host log line

The `introspect:<call_id>` `StepResult.details` records `allow_write: <bool>` on every call and `acknowledged_permissions: [...]` when write mode was used (the ack strings are fixed capability descriptions, not secrets — recorded verbatim). On a write-mode invocation the handler emits one dedicated `logging` line at WARNING (`audit: debug.introspect.run write-mode invocation run_id=… call_id=… acknowledged=…`) so a reviewer can grep every mutation-capable call. The audit detail and log fire for *admitted* write-mode calls (after the gate passes), independent of whether the script actually wrote.
...
4. **Full read-only `Program` proxy installed on every call.** Rejected: a proxy that wraps the `Program` for *all* uses breaks drgn C-level constructors that `isinstance`-check their argument (`drgn.Object(prog, …)`), which read-only scripts use — a direct AC#1 violation. The guard intercepts only the `prog.write` attribute path and leaves the real program reachable for argument-position uses; the policy gate, not the proxy, carries the security weight.
...
8. **Record the ack strings through `Redactor` before persisting.** Rejected: the ack values are a fixed, server-defined allowlist of human-readable capability descriptions (e.g. "mutate live kernel state via drgn write APIs"), not guest output or secrets. Redacting them would obscure the very audit trail AC#3 requires. (Everything that *does* carry guest output — stdout/stderr/tracebacks — stays redacted as before.)
