...
| [0029](0029-postmortem-list-dumps-fetch.md) | `debug.postmortem.list_dumps` + `.fetch`: SSH enumeration probe, scp staging with re-enumerate-and-match, size-based truncation detection, host-side-only analysis | Accepted |
| [0030](0030-base-config-generation.md) | Base kernel config generation: `base_config` ordered make targets, precedence ladder over a source `.config`, override replacement, explicit debug `config_lines` | Accepted |
| [0031](0031-rootfs-image-source-abstraction.md) | Rootfs image-source abstraction (Phase 1): handler-layer `source_kind` gate, `builder` host-prep semantics, per-boot `copy_on_write` qemu-img overlay, default `minimal` flip | Accepted |
| [0032](0032-guest-ip-lease-discovery.md) | Guest-IP discovery from the libvirt lease: best-effort `virsh domifaddr` poll in the boot provider surfaced as a `details` fact, run_tests override gate on unset/loopback `ssh_host` with point-of-use re-validation | Accepted |
| [0033](0033-wait-for-debugger-frozen-boot.md) | Wait-for-debugger frozen boot: `wait_for_debugger` on `TargetProfile`/`BootOverrides` selects gdbstub `wait=on`, frozen boot skips the readiness wait + guest-IP discovery and returns `SUCCEEDED`-frozen pointing at `debug.start_session` | Accepted |
| [0034](0034-run-readiness-preflight.md) | Run-readiness preflight: `host.check_prerequisites` gains build/target/rootfs profile-name params and three pure checks (`kernel.config` derivable-or-present, `rootfs.image` via `resolve_rootfs_source`+existence, `gdbstub.port` injected bind probe); unknown name is a `FAILED` check | Accepted |
| [0035](0035-headless-serial-console-capture.md) | Headless serial-console capture: libvirt `<serial><log file>` tees output to `console_log_path` and the runner tails the file (polling `virsh domstate` for exit) instead of running interactive `virsh console`, which needs a TTY | Accepted |
| [0036](0036-gdb-mi-hardware-breakpoints.md) | gdb/MI kernel breakpoints use `-break-insert -h` (hardware): software breakpoints don't survive a frozen boot's reset-vector insertion and fail on read-only kernel `.text`; bounded by the four x86 debug registers | Accepted |
| [0037](0037-unprivileged-tooling.md) | Unprivileged project tooling: rootless two-stage rootfs builder (`virt-builder` customize → whole-disk-ext4 repack via `virt-tar-out`+`virt-make-fs` with `/etc/fstab` normalization, fail-loud on missing template), base image made independently qemu-readable (0644 + `virt_image_t`), capability-based preflight (`kvm.access`/`libvirt.connect`/`rootfs.builder`), host-side-only no-escalation guard scoped to `scripts/`+`justfile`, two-tier integration test, configurable image dir with a libvirt-readable default; supersedes the build-recipe privilege-model portion of ADR 0031 | Accepted |
| [0038](0038-rootfs-build-input-hardening.md) | `build-rootfs.sh` input hardening: `KDIVE_ROOTFS_SSH_USER` allowlist (closes guest-shell injection + `--ssh-inject` colon misparse), `KDIVE_ROOTFS` symlink refusal + `realpath -m` canonicalization (no fixed-base containment — output path is operator-configurable), scoped `umask 0077` around `mktemp`, intentional-expansion comment on the `guestfish` heredoc | Accepted |

| [0039](0039-introspect-drgn-provenance-failure-taxonomy.md) | Introspect drgn provenance-failure taxonomy: split the catch-all `drgn_version_skew` into `drgn_version_skew` (AttributeError gap) / `drgn_api_incompatible` (other resolution errors) / `provenance_unverifiable` (no build-id), each carrying `error_type` + observed `drgn_version`; no hard version floor | Accepted |
