...
def test_manifest_step_retry_helper_is_store_owned() -> None:
    assert hasattr(artifact_store, "record_step_with_retry")
    assert not hasattr(server, "_record_step_with_retry")
    assert not hasattr(kernel_handlers, "_record_step_with_retry")
