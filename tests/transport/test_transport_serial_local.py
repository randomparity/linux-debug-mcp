...
def test_stop_joins_the_pump_before_closing_the_source_fd(tmp_path, monkeypatch):
    ...
    from kdive.transport.backends import serial_local
...
def test_write_client_retries_on_ewouldblock(tmp_path):
    ...
    from kdive.transport.backends.serial_local import SerialConsoleBridge, _PumpStep
...
class _RecordingProxy:
    def __init__(self):
        from kdive.transport.backends.proxy import ProxyHandle
...
def test_concurrent_attach_to_the_same_source_is_refused(tmp_path):
    ...
    from kdive.transport.backends.serial_local import SerialLocalConflictError
